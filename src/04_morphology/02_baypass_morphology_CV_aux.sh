...
# Name this job
#SBATCH --job-name=baypass_morphology_CV_aux
...
cd $WORKING_FOLDER/morphometrics
...
# This part of the script will check and generate, if necessary, all of the output folders used in the script
if [ -d "CV" ]
then echo "Working CV folder exist"; echo "Let's move on."; date
else echo "Working CV folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/morphometrics/baypass/CV; date
fi

# Change directory 
cd $WORKING_FOLDER/morphometrics/baypass/CV

# This part of the script will check and generate, if necessary, all of the output folders used in the script
if [ -d "aux_mode" ]
then echo "Working aux_mode folder exist"; echo "Let's move on."; date
else echo "Working aux_mode folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/morphometrics/baypass/CV/aux_mode; date
fi

#--------------------------------------------------------------------------------

# Change directory 
cd $WORKING_FOLDER/morphometrics/baypass/CV/aux_mode
...
-efile $WORKING_FOLDER/morphometrics/guide_files/Baypass_morphology_CV.txt \
-d0yij 4 \
-auxmodel \
-outprefix NC_baypass_morphology_CV_aux \
-nthreads 20
