...
baypass_sig_SNPs <- XtX[which(XtX$M_XtX >= pod.thres.99),]
...
plot(XtX$XtXst, xlab="Position", pch=19)
...
plot(XtX$XtXst, xlab="Position", col=ifelse(XtX$M_XtX >= pod.thres.99, "black", "#bebebe93"), pch=19)
abline(h=pod.thres.99, lty=2, col='red') 
dev.off()
...
plot(XtX$log10.1.pval., ylab=expression(-log[10](italic(p))), xlab="Position", 
col=ifelse(XtX$M_XtX >= pod.thres.99, "black", "#bebebe93"), pch=19)
abline(h=pod.thres.99, lty=2, col='red') 
abline(h=-log10(0.05/dim(XtX)[1]), lty=1, col="blue") # Bonferroni threshold
dev.off()

# ================================================================================== #

# Write file of POD significant SNPs
write.table(baypass_sig_SNPs, "data/processed/outlier_analyses/baypass/POD/baypass_sig_SNPs", sep = "\t", row.names=FALSE)
