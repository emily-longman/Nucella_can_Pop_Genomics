...
# Name this job
#SBATCH --job-name=filter_vcf_relaxed_filt
...
if [ -d "vcf_clean_relaxed_filt" ]
then echo "Working vcf_clean_relaxed_filt folder exist"; echo "Let's move on."; date
else echo "Working vcf_clean_relaxed_filt folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_clean_relaxed_filt; date
fi
...
vcftools --vcf $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_clean_relaxed_filt/N.canaliculata_pops_filter_minQ60_maxmissing1.0.recode.vcf \
--FILTER-summary --out $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_clean_relaxed_filt/N.canaliculata_pops_filter_minQ60_maxmissing1.0.recode.vcf
# Generate a file containing the depth per site summed across all individuals
vcftools --vcf $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_clean_relaxed_filt/N.canaliculata_pops_filter_minQ60_maxmissing1.0.recode.vcf \
--depth --out $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_clean_relaxed_filt/N.canaliculata_pops_filter_minQ60_maxmissing1.0.recode.vcf