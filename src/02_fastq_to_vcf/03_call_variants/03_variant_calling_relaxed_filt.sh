...
# Name this job
#SBATCH --job-name=variant_calling_freebayes_relaxed_filt
...
if [ -d "vcf_freebayes_relaxed_filt" ]
then echo "Working vcf_freebayes_relaxed_filt folder exist"; echo "Let's move on."; date
else echo "Working vcf_freebayes_relaxed_filt folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_freebayes_relaxed_filt; date
fi

cd $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_freebayes_relaxed_filt

if [ -d "partitions" ]
then echo "Working partitions folder exist"; echo "Let's move on."; date
else echo "Working partitions folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_freebayes_relaxed_filt/partitions; date
fi

cd $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_freebayes_relaxed_filt/partitions
...
else echo "Working genome.scaffold.names.${SLURM_ARRAY_TASK_ID} folder doesnt exist. Let's fix that."; mkdir $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_freebayes_relaxed_filt/partitions/genome.scaffold.names.${SLURM_ARRAY_TASK_ID}; date
fi
...
freebayes -f $REFERENCE -L $BAMLIST -r ${SCAFFOLD} -K -F 0.01 -C 1 -G 2 --limit-coverage 500 -n 4 -m 30 -q 20 | gzip -c > $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_freebayes_relaxed_filt/partitions/genome.scaffold.names.${SLURM_ARRAY_TASK_ID}/Nucella.freebayes.${SCAFFOLD}.vcf.gz
...
bcftools concat \
$WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_freebayes_relaxed_filt/partitions/genome.scaffold.names.${SLURM_ARRAY_TASK_ID}/*.vcf.gz \
-Oz -o $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_freebayes_relaxed_filt/partitions/genome.scaffold.names.${SLURM_ARRAY_TASK_ID}.vcf.gz
...
rm -R $WORKING_FOLDER/data/processed/fastq_to_vcf/vcf_freebayes_relaxed_filt/partitions/genome.scaffold.names.${SLURM_ARRAY_TASK_ID}
